...
from utils.activations import Mish, Swish
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', type=str, default='./weights/v4-p5-transpose.pth',
                        # parser.add_argument('--weights', type=str, default='./weights/yolov4-p5.pt',
                        help='weights path')  # from yolov5/models/
    parser.add_argument('--img-size', nargs='+', type=int,
                        # default=[896, 896], help='image size')  # height, width
                        default=[416, 416], help='image size')  # height, width
    ...
    for k, m in model.named_modules():
        ...
        if isinstance(m, models.experimental.TransposeBlock) and isinstance(m.act, models.common.Mish):
            m.act = Mish()  # assign activation
        if isinstance(m, models.common.BottleneckCSP) or isinstance(m, models.common.BottleneckCSP2) \
            ...
            if isinstance(m.bn, nn.SyncBatchNorm):
                bn = nn.BatchNorm2d(m.bn.num_features,
                                    eps=m.bn.eps, momentum=m.bn.momentum)
                bn.training = False
    ...
    try:
        ...
        f = opt.weights.replace(
            '.pt', f'-{opt.img_size[0]}-v9.onnx')  # filename
        torch.onnx.export(model, img, f, verbose=False, opset_version=9,  # 12
