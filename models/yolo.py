...
from models.experimental import MixConv2d, CrossConv, C3, TransposeBlock
...
class Detect(nn.Module):
    def __init__(self, nc=80, anchors=(), ch=()):  # detection layer
        ...
        # TODO: 这里进行了修改
        self.no = nc + 5 + 4*2  # number of outputs per anchor
    ...
    #             if not self.export:
    #                 y[..., 0:2] = (
    #                     y[..., 0:2] * 2. - 0.5 + self.grid[i].to(x[i].device)) * self.stride[i]  # xy
    #                 y[..., 2:4] = (y[..., 2:4] * 2) ** 2 * \
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            x[i] = self.m[i](x[i])  # conv
            bs, _, ny, nx = x[i].shape  # x(bs,c,h,w) to x(bs,na,h,w,no)
            ...
            if not self.training:  # inference
                ...
                if not self.export:
                    y = x[i].sigmoid()
                    y[..., 0:2] = (y[..., 0:2] * 2. - 0.5 + self.grid[i].to(x[i].device)) * self.stride[i]  # center xy
                    y[..., 2:4] = (y[..., 2:4] * 2)** 2 * self.anchor_grid[i]  # wh
                    # TODO: hard code here
                    for j in range(4):
                        s = 4 + 2 * j
                        e = 4 + 2 * (j+1)
                        y[..., s:e] = (y[..., s:e] -0.5) * 4 * self.anchor_grid[i]  # keypoints xy
                z.append(y.view(bs, -1, self.no))
    ...
    #     return x if self.training else z if self.export else(torch.cat(z, 1), x)
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):
        ...
        if m in [nn.Conv2d, Conv, Bottleneck, SPP, DWConv, MixConv2d, Focus, CrossConv, BottleneckCSP, BottleneckCSP2, SPPCSP, VoVCSP, C3, TransposeBlock]:
            c1, c2 = ch[f], args[0]
